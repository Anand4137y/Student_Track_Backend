...
export const staffLogin = async (req: Request, res: Response) => {
    ...
    const token = generateToken({ userId: isUser._id!.toString(), schoolId: isUser.school ? isUser.school.toString() : null, role: isUser.role });
...
export const getAllSchools = async (req: Request, res: Response) => {
    try {
        const schools = await School.find({ isEnabled: true, isDeleted: false }).select('schoolName schoolCode _id');
        return res.status(200).json({ status: true, data: schools });
    } catch (error) {
